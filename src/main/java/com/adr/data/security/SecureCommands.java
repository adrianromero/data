...
public class SecureCommands {
    ...
    public final static Sentence[] QUERIES = new Sentence[]{
        new SentenceQuery(
        "SUBJECT_ROLE", //OK
        "SELECT S.NAME, S.DISPLAYNAME FROM PERMISSION P JOIN SUBJECT S ON P.SUBJECT_ID = S.ID JOIN ROLE R ON P.ROLE_ID = R.ID WHERE R.NAME = ? AND S.NAME = ?", "ROLE..PARAM", "SUBJECT..PARAM"),
        new SentenceQuery(
        "USERNAME_BYNAME", // OK
