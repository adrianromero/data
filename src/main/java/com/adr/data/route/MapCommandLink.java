 ...
 * @author adrian
 */
public class MapCommandLink implements CommandLink {
    ...
    public MapCommandLink(CommandLink commandlink, Function<? super Record, ? extends Stream<? extends Record>> mapper, boolean ifemptyex) {
    ...
    public void execute(Header headers, List<Record> l) throws DataException {
        ...
        if (!l2.isEmpty()) {
            commandlink.execute(headers, l2);
        } else if (ifemptyex) {
