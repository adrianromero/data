 ...
 * @author adrian
 */
public abstract class ResponseExecute {
    ...
    public static ResponseExecute read(String value) throws IOException {
        return read(new StringReader(value));
    }
    
    public static final ResponseExecute read(Reader r) throws IOException {
        Loader loader = new StreamLoader(r);
        loader.next();
        String type = CommonParsers.parseWord(loader);
        loader.skipBlanks();
        if (ResponseExecuteSuccess.NAME.equals(type)){
            return ResponseExecuteSuccess.readData(loader);
        } else if (ResponseExecuteError.NAME.equals(type)) {
            return ResponseExecuteError.readData(loader);
        } else {
